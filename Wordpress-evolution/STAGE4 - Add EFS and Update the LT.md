...
For `Name` type `Pingnoran-WP-Content`  
...
In the `Virtual Private Cloud (VPC)` dropdown select `PingnoranVPC`  
...
The file system will start in the `Creating` State and then move to `Available`  
...
Under `Name` enter `/Pingnoran/Wordpress/EFSFSID` 
...
```
EFSFSID=$(aws ssm get-parameters --region ap-southeast-2 --names /Pingnoran/Wordpress/EFSFSID --query Parameters[0].Value)
...
for `Template version description` enter `App only, uses EFS filesystem defined in /Pingnoran/Wordpress/EFSFSID`  
