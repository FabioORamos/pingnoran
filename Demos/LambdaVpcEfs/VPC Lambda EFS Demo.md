...
Welcome to this Demo, where we will get experience of working with VPC Based Lambda using an EFS File system. There are a number of key steps in the demo:-  
...
Click on the `Network` tab and verify that there are mount targets in both `ap-southeast-2a` and `ap-southeast-2b`. This is the EFS file system which both the EC2 webservers and lambda function will load and storage images on.  
...
Select the `PingoVPC` from the `VPC` dropdown  
...
Delete all of the resources created in this demo in reverse order of creation.
